...
export default function HistoryScreen() {
	...
	const { payments = [], bills = [], serviceAccounts = [] } = useAccounts();
	...
	const filteredPayments = useMemo(() => {
		...
		return enhancedPayments.filter(
			(payment) => getPaymentStatus(payment) === selectedFilter,
		);
						...
						<>
							...
							<Animated.View entering={FadeInDown.delay(200)} className="mb-6">
								<View className="flex-row items-center mb-4">
									...
									<ThemedText className="ml-2 text-base font-medium">
										Filter by Status
									</ThemedText>
								</View>
								...
								>
									<View className="flex-row gap-2">
										{(
											["all", "paid", "pending", "failed"] as PaymentFilter[]
										).map((filter) => (
											<Pressable
												key={filter}
											...
											>
												...
												>
													...
													>
														...
														{filter === "all"
															? paymentCounts.all
															: paymentCounts[filter as PaymentStatus]}
														)
													</ThemedText>
												</View>
											</Pressable>
