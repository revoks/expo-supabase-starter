...
export function AccountsProvider({ children }: { children: ReactNode }) {
  ...
  const deleteProperty = useCallback(async (id: number) => {
    ...
    setServiceAccounts(current => current.filter(sa => sa.property_id !== id));
  }, []);
  ...
  const addServiceAccount = useCallback(async (propertyId: number, service: Omit<ServiceAccount, 'id' | 'property_id'>) => {
    setServiceAccounts(current => [...current, {
      ...service,
      id: Date.now(),
  ...
  const updateServiceAccount = useCallback(async (id: number, data: Partial<ServiceAccount>) => {
  ...
  const deleteServiceAccount = useCallback(async (id: number) => {
    setServiceAccounts(current => current.filter(service => service.id !== id));
    // Also remove related bills
    setBills(current => current.filter(bill => bill.service_account_id !== id));
  }, []);

  const getPropertyServices = useCallback((propertyId: number) => {
    return serviceAccounts.filter(service => service.property_id === propertyId);
  }, [serviceAccounts]);

  // Bill operations with enriched data
  const enrichBill = useCallback((bill: Bill): Bill => {
    const serviceAccount = serviceAccounts.find(sa => sa.id === bill.service_account_id);
    const provider = serviceAccount?.provider_id ? providers.find(p => p.id === serviceAccount.provider_id) : undefined;
  ...
  const getBillsByProperty = useCallback((propertyId: number) => {
  ...
  const getBillsByServiceAccount = useCallback((serviceAccountId: number) => {
    return bills
  ...
  // Payment operations
  const createPayment = useCallback(async (billId: number, paySystemId: number) => {
  ...
  const payAllBills = useCallback(async () => {
    const pendingBills = bills.filter(bill => bill.status !== 'paid' && !bill.payed_date);
  ...
  const getMonthlySpending = useCallback(() => {
    ...
    return paidBills.reduce((acc: { month: string; amount: number }[], bill) => {
