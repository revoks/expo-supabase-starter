...
export function AppBar({
  ...
  return (
    <Animated.View entering={FadeIn}>
      <SafeAreaView edges={['top']} className="border-b bg-card/95 border-border backdrop-blur-lg">
        <View className="flex-row items-center justify-between px-4 py-2">
          ...
          {isSearching ? (
            ...
            >
              <TextInput
                ...
                placeholderTextColor={colorScheme === 'dark' ? colors.dark.mutedForeground : colors.light.mutedForeground}
              />
            </Animated.View>
          ) : (
            <ThemedText
          ...
          <View className="flex-row items-end justify-end w-16 gap-2">
            ...
            {rightAction && !isSearching && (
              ...
              >
                {rightAction.icon ? (
                  <ThemedView className={`flex items-center justify-center w-9 h-9 rounded-full ${rightAction.className ?? 'bg-accent/10'}`}>
                    {rightAction.icon}
                  </ThemedView>
                ) : (
                  <ThemedText className={rightAction.className ?? "text-accent font-semibold"}>
                    {rightAction.label}
                  </ThemedText>
                )}
              </Pressable>
