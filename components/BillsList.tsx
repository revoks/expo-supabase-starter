...
export default function BillsList({
	...
	const warningColor =
		colorScheme === "dark" ? colors.dark.warning : colors.light.warning;
	const errorColor =
	...
	return (
		<View className="p-4">
			{bills.map((bill, index) => {
				...
				const providerName =
					bill.service_account?.provider?.title || "Unknown Provider";
									...
									<View className="mb-4">
										<View className="flex-row items-center justify-between py-2">
											<View className="flex-row items-center">
												...
												/>
												<ThemedText className="text-sm text-muted-foreground">
													{bill.service_account?.description ||
														"Service account"}
												</ThemedText>
											</View>
										</View>
