...
import {
	PaymentStatusBadge,
	PaymentStatus,
} from "@/components/ui/PaymentStatusBadge";
...
interface GroupedPayments {
	[key: string]: {
		month: string;
		totalAmount: number;
		payments: (Payment & { bill: Bill; serviceAccount: ServiceAccount })[];
	};
}
...
export function PaymentHistoryList({
	...
	const groupedPayments = payments.reduce((acc: GroupedPayments, payment) => {
		const dateString = payment.paid_at || payment.bill.due_date || payment.create_date;
		const date = new Date(dateString);
	...
	if (payments.length === 0) {
		return (
			<Card
				variant="elevated"
				className={`items-center justify-center p-8 ${className}`}
			>
							...
							>
								<Card variant="elevated" className="overflow-hidden">
										...
										<View className="flex-row items-center justify-between">
											<View className="flex-row items-center flex-1">
												<View className="p-2.5 rounded-xl bg-accent/10">
													<ServiceIcon
														type={
															payment.serviceAccount.provider
												...
												<View className="flex-1 ml-3">
													<ThemedText className="text-base font-semibold">
														{payment.serviceAccount.provider?.name ||
															"Unknown Provider"}
													</ThemedText>
									...
									<View className="p-4">
										...
										<View className="flex-row items-center justify-between">
											<View className="flex-row items-center">
												<Receipt size={16} color={successColor} />
												<ThemedText
													className="ml-2 text-sm font-medium"
													style={{ color: successColor }}
												>
													Paid on{" "}
													{formatDate(payment.paid_at || payment.bill.due_date || payment.create_date)}
												</ThemedText>
											</View>
